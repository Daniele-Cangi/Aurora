...
int main() {
    ...
    assert(comparison.strict.scheduled_turns.front() == 0);
    assert(comparison.strict.turn_gap_bound_violations > 0);
    assert(comparison.strict.maximum_observed_turn_gap_ms >
           comparison.strict.comparison_turn_gap_bound_ms);

    assert(comparison.fair.turn_gap_bound_violations == 0);
    assert(comparison.fair.maximum_observed_turn_gap_ms <=
           comparison.fair.comparison_turn_gap_bound_ms);
    assert(std::all_of(
        comparison.fair.scheduled_turns.begin(),
        comparison.fair.scheduled_turns.end(),
    ...
    assert(custom.fair.comparison_turn_gap_bound_ms == 9'000);
    assert(custom.fair.turn_gap_bound_violations == 0);
    assert(custom.strict.turn_gap_bound_violations > 0);
    ...
    assert(sweep.report.starts_with(
        "AURORA_GENERATION_SCHEDULER_SWEEP_V2\n"));

    auto regressed = comparison.fair;
    regressed.turn_gap_bound_violations = 1;
    assert(!aurora::simulation::generation_scheduler_benchmark_gate_passes(
        regressed));
