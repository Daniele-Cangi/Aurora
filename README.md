...
**Primary current task:** migrate the fixed-step simulator to a deterministic discrete-event kernel, preserving the current V7/V6 paired ledgers as regression oracles before adding further policy surface
...
| Telemetry and replay | Implemented prototype | JSONL engine telemetry, V6 decision traces, V7 simulator event ledgers, canonical contact and V2 generation-arrival schedules, benchmark channel traces, and a strict-vs-fair adversarial scheduler harness; future identities are reserved without invoking policy planning, scheduler turns are distinct from HAL-accepted effective service, and paired replay reconstructs both | The fairness bound applies to scheduler turns, not effective service; concurrent/mobile nodes, imported emulation traces and physical hardware remain outside the current event model |
...
The main simulator now applies the optimizer through a hard `SafetyEnvelope`, transmits the organism's spawned packets, and feeds the same authoritative `DecodeReport` to delivery and transport health. The supervisory monitor reports `NO_EVIDENCE` until active-flow observations exist, excludes inactive classes, applies Schmitt-style enter/exit thresholds, and requires consecutive evidence before changing an established state. `NO_EVIDENCE` forces conservative operation, and recovery from `CRITICAL` must pass through `DEGRADED`. V6 decision traces replay proposal, action and supervisory transitions per action, while the paired V7 simulator ledger independently reconstructs the implemented fixed-node event stream from declared arrivals and contacts through scheduling, harvesting, RIS, channel outcomes, inbox movement and generation state. Future generation IDs are reserved deterministically without calling the policy; the policy-dependent descriptor and initial symbols are created at the declared arrival, after all earlier recorded feedback. Each event identifies the granted scheduler turn and separately records the number of attempts accepted by the HAL as effective transport service. The paired verifier binds that count to the V6 execution trace. This proves the implemented simulation path and its causal ordering, not that the configured thresholds are calibrated.
...
The engine deterministically reserves each generation identity without invoking policy planning. At the declared arrival it calls the current policy, constructs the immutable descriptor, spawns initial symbols and binds the completed plan into the V7 ledger before recording the arrival event. Feedback from A can therefore change B's descriptor and symbol budget while B retains its precomputable identity. `--generation-scheduler fair` is the default: it selects by effective class (`critical`, then `important`, then `elastic`), earliest absolute descriptor expiry, arrival time and stable schedule index. A generation waiting without a scheduled turn is promoted by one class every `--generation-aging-ms` (default 2000 ms). After `--generation-starvation-ms` (default 3000 ms) it enters the anti-starvation lane, which is ordered by earliest fairness deadline before EDF and the stable tie-breaks. Every selected 1000 ms quantum resets that generation's scheduling-turn clock. A later critical or tighter-deadline generation can therefore preempt an earlier generation, while an elastic generation still receives a bounded turn and remains independently decodable across preemption and resume.
...
Aurora-X should be judged neither as a finished product nor as a small disposable prototype. Its generation loop and first synthetic channel campaigns are coherent, modular and replayable; multi-generation policy planning is causal at arrival, and scheduling opportunity is now distinct from HAL-accepted effective service. The next obligation is to replace the fixed-step loop with a deterministic discrete-event kernel using the current V7/V6 paired ledgers as regression oracles. Only then should the project build a complete contact/deadline-aware end-to-end transport benchmark, add an established external FEC baseline, move to process-separated emulation with an explicit reverse-feedback channel, and finally pursue calibrated hardware evidence.
