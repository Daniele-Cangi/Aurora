...
**Primary current task:** add reverse-path impairments and obtain explicit remote-host execution evidence
...
| Process emulation | Implemented deterministic loopback slice | Separate sender/receiver processes multiplex two generations over versioned UDP frames, replay checksum-bound pass/drop/duplicate/delay events, expose independent IPv4 bind/destination endpoints, and return per-generation policy feedback | Evidence remains loopback-only; reverse-path impairment, authenticated provenance, remote-host execution and calibrated timing remain future work |
...
`aurora_process_emulation` runs sender and receiver roles in separate OS processes. The sender multiplexes two concurrent generations by interleaving their canonical descriptors and FEC symbols on one IPv4 UDP destination. The receiver keeps decoder-only state keyed by generation identity, without access to either source payload, and returns progress or terminal `DecodeReport` summaries to a separately configured feedback destination. The sender validates and applies each successful terminal feedback exactly once to its adaptive policy. Bind and destination IPv4 literals are independent, so the same roles can be configured across hosts; the checked-in CI evidence still uses loopback.

Forward symbol attempts are driven by a checked-in impairment trace. V1 binds cyclic `P` (pass), `D` (drop) and `U` (duplicate) actions to a scenario name and FNV-1a checksum. V2 uses `action@delay-ms` events, capped at 60 seconds. Events index the global interleaved symbol-attempt order; release time is ordered first and attempt index breaks ties. A delayed earlier event can therefore be overtaken by a later immediate event reproducibly. Descriptor retransmission and the reverse channel remain outside this impairment model.

Descriptor, symbol and feedback frames share a bounded versioned envelope with explicit type, payload length and checksum. This detects accidental corruption and incompatible framing; it is not authentication. The current harness uses two generations and a replayable pass/drop/duplicate/delay profile on loopback. It proves process and channel separation plus deterministic multiplexing and release ordering, not Internet behaviour, remote-host success, calibrated timing or field performance.
...
```bash
./build/bin/aurora_process_emulation receiver 127.0.0.1 47001 127.0.0.1 47002 2
...
```bash
python tests/run_process_emulation.py ./build/bin/aurora_process_emulation benchmarks/process_timed_v2.trace
```
