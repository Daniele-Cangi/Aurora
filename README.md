...
**Primary current task:** extend authenticated independent-VM evidence beyond an overlay and calibrate transport timing
...
| Process emulation | Implemented authenticated independent-VM slice | Separate sender/receiver processes multiplex two generations over direction/session-bound process frames, replay independent forward and reverse impairment traces, reject replays with a reorder-tolerant window, expose independent IPv4 endpoints, and apply monotonic policy feedback; CI exercises both distinct Docker namespaces and two fresh GitHub-hosted Ubuntu VMs | Real HMAC requires `USE_SODIUM=ON`; current independent-VM evidence traverses a declared Tailscale overlay (DERP is permitted and recorded), so raw public routing, controlled physical hosts and calibrated timing remain future work |
