...
**Current status:** the codec, generation manager and transport policy are now separate interfaces. Fixed and biological policies are injectable and deterministic. Threshold, PID-style and risk-sensitive controllers remain pending. The V6 decision trace reconstructs contact-aware proposal, action and supervisory transitions. A paired V3 simulator ledger reconstructs the current fixed-node stream, including canonical external generation arrivals, FIFO active-generation selection, declared contact windows, harvesting, ingest, RIS, channel history, global RNG use and generation-qualified packet arrivals. Alternative priority/deadline schedulers and concurrent/mobile nodes remain pending.
...
The current V6 `DecisionReplayLog` derives each proposal from recorded channel summaries, UCB/channel-selector state, epoch and isolated RNG state; verifies contact-aware safety decisions; replays each admitted action; applies UCB feedback; and reconstructs the bounded supervisory transition. The paired V3 `SimulationEventLedger` embeds canonical `ContactSchedule` and `GenerationArrivalSchedule` inputs, binds records to fixed node topology and immutable generation identities, then independently reconstructs timed generation release, FIFO active-generation selection, contact availability, inter-step harvesting/ingest, RIS phases, world gain, channel history, LBT/fading/pacing randomness, generation-qualified inbox arrivals and per-generation state continuity. This covers multiple external arrivals in the implemented fixed-node simulator. Concurrent/mobile nodes, alternative schedulers and imported field/emulation traces remain pending.
