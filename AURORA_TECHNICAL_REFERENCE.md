# Aurora technical reference

> Long-form architecture, build, protocol, experiment and validation reference.
> For the concise project overview, start with the [README](README.md).
...
terminal completion acknowledgement; study-v3 stopped at sequence 2 before
sender dispatch when an attached receiver SSH/IAP connection closed. The
controller-resilience amendment launches the receiver independently of any one
SSH session and adds bounded exact-name cleanup retries. See
[`raw-host-policy-pilot-v1-controller-resilience-amendment`](docs/raw-host-policy-pilot-v1-controller-resilience-amendment.md).
No lifecycle from a stopped identity can be reused.
